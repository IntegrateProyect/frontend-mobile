...
class _StudentHomeScreenState extends State<StudentHomeScreen> {
  ...
  Future<void> _refreshAllData() async {
    ...
    // Disparamos las cargas de todos los providers especializados
    await Future.wait([
      context.read<StudentHomeProvider>().loadHomeData(),
      context.read<StudentGroupProvider>().loadGroupData(),
      context.read<StudentAppointmentsProvider>().loadAppointments(),
      context.read<StudentAnnouncementsProvider>().loadAnnouncements(),
      context.read<StudentProgressProvider>().loadProgressData(),
  ...
  Future<bool> _validateStudentGroup() async {
    final bool canContinue = await requireStudentGroup(
      context: context,
    );
  ...
  Future<void> _openVocationalGames() async {
    final bool canContinue = await _validateStudentGroup();
  ...
  Future<void> _openChatbot() async {
    ...
    final StudentHomeProvider provider = context.read<StudentHomeProvider>();
  ...
  Future<void> _openResults() async {
    ...
    final progressProvider = context.read<StudentProgressProvider>();

    if (progressProvider.hasVocationalResults) {
  ...
  Future<void> _showResultsInformation() async {
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (bottomSheetContext) {
        final Color titleColor = isDark ? Colors.white : StudentUiColors.darkText;
        final Color secondaryColor = isDark ? Colors.grey.shade400 : Colors.grey.shade600;
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: isDark ? const Color(0xFF282443) : const Color(0xFFF0EEFA),
                  borderRadius: BorderRadius.circular(24.r),
                ),
                child: Icon(
                  Icons.bar_chart_rounded,
                  color: isDark ? const Color(0xFFB59AFF) : const Color(0xFF756EB2),
                  size: 40.sp,
                ),
              ),
  ...
  Widget _buildResultInfoItem({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: isDark ? const Color(0xFF242539) : const Color(0xFFF8F8FB),
      ...
      child: Row(
        children: [
          CircleAvatar(
            radius: 13.r,
            backgroundColor: isDark ? const Color(0xFF34304E) : const Color(0xFFEDEDF3),
            child: Text(
              number,
              style: TextStyle(
                fontSize: 10.sp,
          ...
          Expanded(
            child: Text(
              text,
              style: TextStyle(
                color: isDark ? Colors.white70 : StudentUiColors.darkText,
                fontSize: 12.sp,
  ...
  Widget build(BuildContext context) {
    final homeProvider = context.watch<StudentHomeProvider>();
    final groupProvider = context.watch<StudentGroupProvider>();
    final progressProvider = context.watch<StudentProgressProvider>();
    final appointmentsProvider = context.watch<StudentAppointmentsProvider>();
  ...
  Widget _buildBody({
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          StudentGreeting(
            name: homeProvider.firstName,
            groupName: groupProvider.hasGroup ? groupProvider.currentGroupName : null,
            counselorName: groupProvider.hasGroup ? groupProvider.currentCounselorName : null,
          ),
          SizedBox(height: 18.h),
          VocationalRouteCard(
            hasGroup: groupProvider.hasGroup,
            gamesStarted: progressProvider.hasStartedGames,
            gamesCompleted: progressProvider.hasCompletedGames,
            chatbotCompleted: homeProvider.hasChatbotInteraction,
            resultsCompleted: progressProvider.hasVocationalResults,
          ...
          StudentAppointmentsSection(
            appointments: groupProvider.hasGroup ? appointmentsProvider.appointments : const [],
            onRefresh: _refreshAllData,
          ),
          SizedBox(height: 20.h),
